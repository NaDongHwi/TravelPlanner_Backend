...
public class AiService {
    ...
    public AiRouteResponse evaluateAndModifyRoute(String userContext, List<Place> draftRoute, String lang,
        ...
        String prompt = "너는 10년 차 전문 일본 여행 플래너야. " +
                ...
                "4. [지능형 식음료(F&B) 배치 규칙] 사용자의 테마에 '맛집'이나 '식도락'이 포함되어 있다면, 현지 맛집, 길거리 음식, 야식, 디저트 카페 등을 하루 4~5번까지 적극적으로 배치해. 단, 아무리 식도락 테마라도 3시간 이내에 무거운 식사(식당)를 연속으로 2번 욱여넣는 등 인간의 위장으로 소화 불가능한 일정은 절대 금지야. 만약 맛집 테마가 아니라면 하루 2번(점심, 저녁)의 식사와 1번의 카페 정도로 제한해.\n" +
                ...
                "9. [지능형 동선 반경 및 근교 확장] 3일 이하의 짧은 일정이라면 숙소에서 너무 먼 '지리적 이상치'는 과감히 버려. 하지만 4박 이상의 장기 일정이라면 반경을 과감히 넓혀서 사용자가 입력한 목적지 주변의 핵심 근교 소도시(예: 오사카의 경우 교토/나라, 후쿠오카의 경우 유후인/벳푸, 삿포로의 경우 비에이/노보리베츠 등)를 너의 방대한 지식망에서 직접 찾아 무조건 일정에 융합시켜야 해.\n" +
                "10. [자율적 동적 제안 및 데이터 극복] 내가 제공한 80개의 [임시 동선] 데이터 풀에 절대 얽매이지 마! 특히 후반부 일정(6일 차 이상)이 도심 쇼핑몰이나 공원으로만 획일화되는 것을 엄격히 경계해. 사용자의 목적지와 테마(예: 자연, 온천, 힐링 등)에 완벽히 부합하지만 내 임시 동선 데이터에는 빠져있는 '해당 지역 최고의 외곽 자연/온천 랜드마크'가 있다면, 망설이지 말고 타임라인에 자유롭게 추가하여 일정의 퀄리티를 끝까지 끌어올려.\n\n" +
                ...
                "13. [계절 및 특별 이벤트 반영] 고객의 여행 시작일(" + startDate + ")과 총 기간(" + totalDays + "일)을 네가 가진 일본 지역별 데이터와 연동하여 정밀 분석해! 해당 시기와 지역에 맞는 계절적 명소(예: 봄의 벚꽃, 가을의 단풍, 겨울의 겨울산 뷰)나 전통 축제(마츠리, 불꽃놀이 등)가 있다면, 기존 임시 동선 데이터를 뛰어넘더라도 타임라인에 무조건 1개 이상 적극적으로 추가하여 제안해야 해.\n" +
                "14. [지능형 숙소 다변화 및 역제안 규칙] 사용자의 여행이 4박 이상인 장기 여행일 경우, '도심 내 단일 숙소 체류'가 사용자의 '핵심 테마'와 충돌하는지 비판적으로 판단해. 예를 들어 테마가 '온천/힐링/자연'인데 긴 일정 내내 도심 비즈니스 호텔만 배정된다면 심각한 기획 오류야. 네가 가진 지식을 활용해 목적지에 맞는 근교 특화 지역을 스스로 도출하여 '거점 분할'을 기획해. (예: 목적지가 후쿠오카면 유후인/벳푸 료칸, 홋카이도면 노보리베츠, 도쿄면 하코네, 시즈오카면 슈젠지/아타미 등). 이처럼 테마에 최적화된 근교 숙박 일정을 타임라인에 1~2박 강제로 배치하거나, 최종 결과 설명(`reason`) 칸에 '선택하신 [특정 테마]를 극대화하기 위해 X일차에는 [AI가 찾은 근교 지역명]에서의 숙박으로 일정을 찢는 것을 강력 추천합니다'라고 인간 전문가처럼 반드시 제안해.\n\n" +
                "15. [지능형 쇼핑/실내 일정 분배 규칙] 사용자의 테마에 '쇼핑'이 포함되어 있지 않다면, 하루에 백화점이나 대형 쇼핑몰을 2회 이상 연속으로 중복 배치하는 것을 엄격히 금지해. 실내 일정이 필요하다면 쇼핑몰 1곳, 박물관/미술관 1곳, 전통 찻집 1곳 등으로 카테고리를 다채롭게 분산시켜. (단, '쇼핑' 테마를 명시적으로 선택한 유저라면 이 제한 없이 자유롭게 쇼핑 시설을 연속 배치해도 좋아.)\n\n" +
