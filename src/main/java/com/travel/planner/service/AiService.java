...
public class AiService {
    ...
    public AiRouteResponse evaluateAndModifyRoute(String userContext, List<Place> draftRoute) {
        ...
        String prompt = "너는 10년 차 전문 일본 여행 플래너야. " +
                ...
                "9. [입출국 시간 기반 스케줄링 통제] [고객 정보]에 포함된 입국/출국 시간을 반드시 확인해!\n" +
                "   - 입국 시간이 '오후'나 '저녁'일 경우, 1일 차 일정은 숙소 체크인과 가벼운 저녁 식사, 야경 위주로만 여유롭게 배치해. 절대 1일 차 아침이나 낮에 명소 방문을 넣지 마.\n" +
                "   - 출국 시간이 '오전'일 경우, 마지막 날은 명소 방문 없이 공항 이동 시간만 고려하여 일정을 비워둬.\n" +
                "   - 시간이 '미정'일 경우에는 가장 꽉 찬 풀(Full) 일정 기준으로 자유롭게 구성해.\n\n" +
